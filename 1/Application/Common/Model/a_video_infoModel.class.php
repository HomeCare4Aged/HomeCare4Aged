...
class a_video_infoModel extends Model{
	...
	array('video_length','require','视频时长不能为空！'),
	array('send_datetime','require','发布时间不能为空！'),
	array('sender_id','require','发布人不能为空！'),
	array('video_type_id','require','视频类别不能为空！'),
	);
	...
	public function getArticles($cond=array(),$page,$pageSize){
		...
		$list = M('a_video_info')->limit($offset,$pageSize)
		->join('a_video_type on a_video_info.video_type_id=a_video_type.video_type_id')
		->where($cond)
